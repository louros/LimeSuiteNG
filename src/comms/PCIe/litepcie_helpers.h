...
// Modify checked_ioctl to return the status from _check_ioctl
    #define checked_ioctl(fd, op, data) \
        (DeviceIoControl(fd, op, &(data), sizeof(data), &(data), sizeof(data), NULL, NULL) \
                ? (_check_ioctl(0, __FILE__, __LINE__), 0) \
                : (_check_ioctl(1, __FILE__, __LINE__), 1))
